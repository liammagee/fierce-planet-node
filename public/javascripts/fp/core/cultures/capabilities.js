...
/*
MoveCapability.prototype = new Capability();
MoveCapability.prototype.constructor = MoveCapability;

MoveCapability.prototype.moveTo = function(agent) {
    agent.x = x; agent.y = y;
    agent.incrementMoves();
};

MoveFreelyCapability.prototype = new MoveCapability();
MoveFreelyCapability.prototype.constructor = MoveFreelyCapability;
function MoveFreelyCapability() {}
...
(function() {
    ...
    this.exercise = function(agent, level) {
        return this.findPositionWithFreeNavigation(level, agent);
    };

    this.findPositionWithFreeNavigation = function(level, withNoRepeat, withNoCollision, withOffscreenCycling) {
        var x = agent.x;
        var y = agent.y;
        var newX = x;
        var newY = y;
        ...
        if (candidateCells.length > 1) {
            ...
            if (this.canCommunicateWithOtherAgents) {
                for (var key in agent.memoriesOfPlacesVisitedByOtherAgents) {
...
}).apply(MoveFreelyCapability.prototype);


MoveToExitCapability.prototype = new MoveCapability();
MoveToExitCapability.prototype.constructor = MoveToExitCapability;
function MoveToExitCapability() {}

(function() {
    this.MAX_DEPTH = 1000;

    this.exercise = function(agent, level) {
        return this.criticalPath(level, agent.x, agent.y);
    };
...
}).apply(MoveToExitCapability.prototype);
